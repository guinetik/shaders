...
/** A single commons library file included by a shader */
export interface CommonsSource {
  /** Commons file name without extension (e.g. 'sphere', 'noise-value') */
  name: string;
  /** Full GLSL source of the commons file */
  source: string;
}
...
export interface ShaderEntry extends ShaderMeta {
  ...
  /** GLSL source code per render pass (raw, without commons prepended) */
  passes: ShaderPasses;
  /** Channel wiring between passes */
  channels: ShaderChannels;
  /** Shared GLSL library files used by this shader */
